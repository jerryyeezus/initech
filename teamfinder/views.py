...
class GenAlgorithm():
    ...
    def get_children(self, state):
        ...
        for i, j in itertools.combinations(range(len(teams)), 2):
            if len(teams[i].members) + len(teams[j].members) > 5:
                continue
        ...
        # could be none avail
        if not did_reduce and not did_shift:
            next_student = cstate.available[0]
...
class GenerateTeams(APIView):
	...
	result = algo.search()
        ...
        teams = assignment.teams.all()
        """
	return Response(serializers.serialize('json', list(assigned_teams)), status=status.HTTP_201_CREATED)
